...
export function InventoryView() {
  ...
  return (
    <>
      <PageHeader eyebrow="Capability catalog" title="Inventory" description="Search and review every skill, plugin, CLI tool, MCP server, and design capability in the workspace." actions={<div className="button-group"><button className="button secondary" type="button" onClick={() => exportData("csv")}><Icon name="database" size={17} />Export filtered CSV</button><button className="button secondary" type="button" onClick={() => exportData("json")}>Export filtered JSON</button></div>} />
      ...
      {response.loading ? <LoadingState label="Loading inventory…" /> : response.error ? <ErrorState message={response.error} onRetry={response.reload} /> : (
        <section aria-labelledby="inventory-results">
          <div className="section-heading compact-heading"><div><h2 id="inventory-results">Capabilities</h2><p id="inventory-results-count" className="result-count" aria-live="polite">{normalized.total.toLocaleString("en-US")} result{normalized.total === 1 ? "" : "s"}</p></div></div>
  ...
  return (
    <>
      <div className="data-table-wrap">
        <table className="data-table" aria-describedby="inventory-results-count"><caption>Filtered capability inventory with owner, status, freshness, and source links.</caption><thead><tr><th scope="col">Capability</th><th scope="col">Type</th><th scope="col">Owner</th><th scope="col">Status</th><th scope="col">Freshness</th><th scope="col">Source</th></tr></thead><tbody>{assets.map((asset) => <tr key={asset.id}><td><strong>{asset.name}</strong><small>{asset.description || "No description provided."}</small></td><td><span className="type-badge">{asset.type}</span></td><td>{asset.owner || "Unassigned"}</td><td><AssetStatus asset={asset} /></td><td><span className={`status-badge ${isStale(asset.updated_at ?? asset.created_at) ? "warning" : "success"}`}>{isStale(asset.updated_at ?? asset.created_at) ? "Stale" : "Verified"}</span><small>{formatDate(asset.updated_at ?? asset.created_at, { month: "short", day: "numeric", year: "numeric" })}</small></td><td>{isSafeExternalUrl(asset.source) ? <a className="external-link" href={asset.source} target="_blank" rel="noopener noreferrer" aria-label={`Open source for ${asset.name} in a new tab`}>Open <span className="sr-only">in a new tab</span> <Icon name="external" size={14} /></a> : <code>{asset.source || "Not available"}</code>}</td></tr>)}</tbody></table>
      </div>
      <div className="mobile-card-list">{assets.map((asset) => <article className="entity-card" key={asset.id}><div className="entity-card-header"><div><span className="type-badge">{asset.type}</span><h3>{asset.name}</h3></div><AssetStatus asset={asset} /></div><p>{asset.description || "No description provided."}</p><dl className="entity-details"><div><dt>Owner</dt><dd>{asset.owner || "Unassigned"}</dd></div><div><dt>Freshness</dt><dd>{isStale(asset.updated_at ?? asset.created_at) ? "Stale" : "Verified"}</dd></div><div><dt>Version</dt><dd>{asset.version || "Not available"}</dd></div></dl>{isSafeExternalUrl(asset.source) && <a className="button secondary full-width" href={asset.source} target="_blank" rel="noopener noreferrer" aria-label={`Open source for ${asset.name} in a new tab`}>Open source <span className="sr-only">in a new tab</span> <Icon name="external" size={16} /></a>}</article>)}</div>
    </>
  );
}
